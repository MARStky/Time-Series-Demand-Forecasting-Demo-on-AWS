...
export function DemandForecastingDemo() {
  const [historicalData, setHistoricalData] = useState<DataPoint[]>(() => {
    // Generate initial data with categories
    const allCategoryData = generateCategoryData(24).flatMap((category) => category.data)
    console.log("Initial historical data with categories:", allCategoryData)
    return allCategoryData
  })

  const [forecastData, setForecastData] = useState<DataPoint[]>(() => {
    // Generate initial forecast for all categories
    const allCategoryData = generateCategoryData(24).flatMap((category) => category.data)
  ...
  useEffect(() => {
    if (historicalData && historicalData.length > 0) {
      try {
        ...
        // Filter historical data by category if selected
        ...
        const newForecast = generateForecastData(dataToForecast, testPeriods, {
  ...
  const handleGenerateNewData = () => {
    ...
    try {
      ...
      const allCategoryData = generateCategoryData(24).flatMap((category) => category.data)
      console.log("Generated new historical data with categories:", allCategoryData)

      setHistoricalData(allCategoryData)
      setTestResults(null)
      setImportError(null)
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            ...
            {/* Removed help text as requested */}
          </CardContent>
